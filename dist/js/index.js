...
var Settings = class {
  constructor(config = {
    ...
    if (!LocalStorage_default.Exists(this.config.storage_key)) {
      LocalStorage_default.Create(this.config.storage_key, {
        settings: {}
      });
    }
  }
...
(0, import_jquery.default)(() => {
  ...
  sidebar_buttons.forEach((btn) => {
    $(btn).on("click", async (e) => {
      ...
      const func = e.target.attributes.getNamedItem("data-function"), _response = await Request_default.new(func, Editor_default.GetValue(), client_default.options);
