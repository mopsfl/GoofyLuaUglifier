...
var Request_default = {
  async new(func, code, options) {
    ...
    return await fetch(`${options.api_url()}${function_name}`, { method: "POST", body: code }).catch((error) => {
      const _error = error;
      Editor_default.SetValue(`${_error.message}`);
      throw error;
    }).finally(() => {
...
(0, import_jquery.default)(() => {
  ...
  sidebar_buttons.forEach((btn) => {
    $(btn).on("click", async (e) => {
      ...
      const func = e.target.attributes.getNamedItem("data-function"), _request = await Request_default.new(func, Editor_default.GetValue(), client_default.options);
      if (_request instanceof Response) {
        ...
        if (_request.ok) {
          ...
          const error_message = `--[[
                    ...
                    ]]${Editor_default.GetValue().replace(/--\[\[(.|\n)*]]/gm, "")}


`.replace(/^\s+/gm, "");
          Editor_default.SetValue(error_message);
        }
      }
